...
describe("getTaskAuditHistory", () => {
  ...
  test("returns 200 with empty audit history when no records exist", async() => {
    mockedProjectFindOne.mockResolvedValue({
      id:1,
      userId:1,
    });// fake database found the project
  ...
  test("returns 200 with audit history successfully", async() => {
    ...
    mockedAuditFindAll.mockResolvedValue(auditHistory);// fake database returns the audit records
  ...
  test("gets audit history only for the requested task and newest first", async() => {
    mockedProjectFindOne.mockResolvedValue({
      id:1,
      userId:1,
    });
    ...
    mockedAuditFindAll.mockResolvedValue([]);
  ...
  test("passes unexpected errors to next", async() => {
    ...
    mockedProjectFindOne.mockRejectedValue(error);// fake database throws an unexpected error
    const req = {
      params: {
        projectId: "1",
        taskId: "2",
